...
private:
    ...
    multimap<int, string> *idMapping;        //Declared a reference to the object. The object is not creatd. it can be later created using new and then it will be allocated on heap. This will require explicit deletion in destructor.
};

myMMap::myMMap(){
    idMapping = new multimap<int, string>;
}
...
void myMMap::add_to_map(int x, int y ){
    pair<int, int> p(x,y);
    aMap.insert(p);
}

void myMMap::add_to_map(int id, string name){
    pair<int, string> p;
    p.first = id;
    p.second = name;
    idMapping->insert(p);
...
void myMMap::remove_value_from_map(string name){
...
void myMMap::remove_key_from_map(int key){
...
void myMMap::swap(){
    multimap<int, string> kFamily = {{7, "Abbu Ji"}, {1, "K"}, {2, "M"}, {3, "G"}, {4, "S"}, {5, "Q"}, {6, "B"}};
    idMapping->swap(kFamily);
}

void myMMap::populate_map(){
...
void myMMap::clear_map(){
    aMap.clear();
    idMapping->clear();
    cout << "Map cleared " << endl;
}

void myMMap::print_map(){
