...
static void start_game_with_defaults();
static UINT toss_for_first_turn();
static char chose_starting_char(UINT pid);
static void draw_board();
static void clean_board();
static void play(UINT starting_player);
static void display_input_choices();

static void player_move(UINT pid);
static UINT toggle_turn(UINT pid);
static bool check_win(UINT pid);
static bool check_tie();
...
char chose_starting_char(UINT pid){
	...
	/* Choose the symbol if the winner of the toss was a human */
...
void play(UINT starting_player){
	...
	do
	{
		...
		// Draw board only after both the players have taken the turn
...
void player_move(UINT pid){
	...
	//Keep waiting until user makes a valid choice	 (Due to random number generator, there is a potential for a deadlock here
	while(true){
		if(pid == Player1_ID){
			cin >> choice;
		}
...
void prog6_3(int argc, char** argv){
